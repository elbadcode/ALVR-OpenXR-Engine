...
const float3 D65_WHITE = float3(0.95045592705, 1.0, 1.08905775076);
//                          0.3457/0.3585  1.0  (1.0-0.3457-0.3585)/0.3585
const float3 D50_WHITE = float3(0.96429567643, 1.0, 0.82510460251);


float3 WHITE = D65_WHITE;

// sRGB / ITU-R BT.709 spec
const float3 LUMA_float = float3(0.2126, 0.7152, 0.0722);
...
// P3<->XYZ
float3 P3LINEAR_TO_XYZ(float3 p3linear) {
    return p3linear * P3LINEAR_TO_XYZ_M;
}
float3 XYZ_TO_P3LINEAR(float3 xyz) {
    return xyz * XYZ_TO_P3LINEAR_M;
}
// Display P3 uses the sRGB TRC (gamma function).
// It also uses the D65 white point.
float3 P3LINEAR_TO_DISPLAYP3(float3 p3linear) {
    return float3(COMPAND_RGB(p3linear.x), COMPAND_RGB(p3linear.y), COMPAND_RGB(p3linear.z));
}
float3 DISPLAYP3_TO_P3LINEAR(float3 displayp3) {
     return float3(UNCOMPAND_SRGB(displayp3.x), UNCOMPAND_SRGB(displayp3.y), UNCOMPAND_SRGB(displayp3.z));
}
float3 XYZ_TO_DISPLAYP3(float3 xyz) {
    return P3LINEAR_TO_DISPLAYP3(XYZ_TO_P3LINEAR(xyz));
}
float3 DISPLAYP3_TO_XYZ(float3 xyz) {
...
// Output format: float3(luma, blue-difference chroma, red-difference chroma)
...
float3 SRGB_TO_SYCC(float3 srgb) {
    return srgb*SRGB_TO_SYCC_M;
}
float3 SYCC_TO_SRGB(float3 sycc) {
    return sycc*SYCC_TO_SRGB_M;
}

// Composite function one-liners
//float3 SRGB_TO_XYZ(float3 srgb) { return RGB_TO_XYZ(SRGB_TO_RGB(srgb)); }
float3 SRGB_TO_XYZ(float3 srgb) { return RGB_TO_XYZ(srgb); } //we already did this step

float3 XYZ_TO_SRGB(float3 xyz)  { return RGB_TO_SRGB(XYZ_TO_RGB(xyz));  }

float3 SRGB_TO_LAB(float3 srgb) { return XYZ_TO_LAB(RGB_TO_XYZ(srgb)); } //we already did this step
...
// Returns chromaticity float2 (x/y, no luminance) in xyY space.
...
float2 PLANCKIAN_LOCUS_CUBIC_XY(float T) {
...
float XYY_MCCAMY_COLOR_TEMPERATURE(float3 xyY) {
    float n = (xyY.x - 0.3320)/(0.1858 - xyY.y);
    return 449.0*n*n*n + 3525.0*n*n + 6823.3*n + 5520.33;
}
float XYZ_MCCAMY_COLOR_TEMPERATURE(float3 XYZ) {
    float3 xyY = XYY_TO_XYZ(XYZ);
...
float XYZ_DELTA_E_CIE2000(float3 xyz1, float3 xyz2) {
    return LAB_DELTA_E_CIE2000(XYZ_TO_LAB(xyz1), XYZ_TO_LAB(xyz2));
}
 
float SRGB_DELTA_E_CIE2000(float3 srgb1, float3 srgb2)  {
...
float SRGB_PERCEPTUAL_LIGHTNESS_DE2000(float3 srgb) {
    return LAB_DELTA_E_CIE2000(SRGB_TO_LAB(srgb), float3(0.0, 0.0, 0.0));
}

// Just returns the L* component after L*a*b* conversion
// Warning: resultant L* is in L*a*b* space (0 to 100)
float SRGB_PERCEPTUAL_LIGHTNESS_LAB(float3 srgb) {
...
float SRGB_LUMA(float3 srgb) {
    return dot(srgb, LUMA_float);
}
float RGB_RELATIVE_LUMINANCE(float3 rgb) {
    return dot(rgb, LUMA_float);
}

// if you use this function and want to display it, make sure you gamma-compress the result with COMPAND_RGB(float x)
float SRGB_RELATIVE_LUMINANCE(float3 srgb) {
    return dot(SRGB_TO_RGB(srgb), LUMA_float);
}
