...
vec3 SRGB_TO_XYZ(vec3 srgb) { return RGB_TO_XYZ(srgb); } //we already did this step

vec3 XYZ_TO_SRGB(vec3 xyz)  { return RGB_TO_SRGB(XYZ_TO_RGB(xyz));  }

vec3 SRGB_TO_LAB(vec3 srgb) { return XYZ_TO_LAB(RGB_TO_XYZ(srgb)); } //we already did this step
